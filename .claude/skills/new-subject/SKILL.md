...
5. **답의 형태** — 최종 잠금은 세 단계 중 고른다 (README "최종 잠금은 세 단계다" 참조):
   ① 숫자 키패드(`gate.code`) ② 라벨 키패드(`gate.keys` — 자모·한자·연도·시대명·기호,
   `freq`는 안 줘도 된다) ③ **콘솔 퍼즐**(gate 없이 최종 퍼즐의 `reward.event`가 곧 열쇠 —
   연표·자모 조합·주기율표처럼 교과 고유의 입력이 필요할 때).
   ③을 고르면 방 개요의 "코드 조각 배정표"는 "각 퍼즐이 공개하는 정보 배정표"로 읽는다.
...
**퍼즐 정답에는 결정적 구조를 넣어라** — 순서·대조·짝맞추기처럼 맞고 틀림이
함수로 판정되는 것. 그래야 `autoplay.ts`가 유일해·수렴을 검산할 수 있다
(한국사 층위 퍼즐이 실증: 전 순열 유일해 + 교정 반복의 수렴 상한을 순수 함수로 증명).
자유 서술형 답은 검산이 불가능해 실플레이에만 기대게 된다.
