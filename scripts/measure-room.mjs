 ...
 *
 * 사용: node scripts/measure-room.mjs <room.png> <N타일>
 */
import { readFileSync } from "node:fs";
import { chromium } from "@playwright/test";

const [, , inPath, nArg] = process.argv;
const N = Number(nArg);
...
const westX = m.minX + INSET.left;
const eastX = m.maxX + INSET.right;
const southY = m.maxY + INSET.bottom;
const midY = (m.westY + m.eastY) / 2 + INSET.mid;
const topY = 2 * midY - southY;
